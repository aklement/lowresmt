c <- read.table(file="lesionfeaturetypeSmall.txt", sep = "\t", head=TRUE)

x <- c$BLEU
#names(x) <- c$Title

colorz <- c("orange", "deepskyblue", "deepskyblue", "deepskyblue", "deepskyblue", "blue", "green", "green")
fillz <- c(15, 15, 15, 15, 15, 15, 15, 15)
anglez <- c(45, 45, 45, 45, 45, 45, 45, 45)
spacez <- c(0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 1.2)

saxis <- 1.3
slabels <- 1.3
snames <- 1.3

pdf(file="lesionfeaturetyperawSmall.pdf", height=4, width=11, onefile=TRUE, family='Helvetica', pointsize=12)
