...
\newcite{Carbonell2006} described a data-driven MT system that used no parallel text. It produced translation lattices using a bilingual dictionary and scored them using an n-gram language model.  Their method has no notion of translation similarity aside from a  bilingual dictionary.
%
\newcite{ravi-knight:2011:ACL-HLT} treat MT without parallel training data as a decipherment task.  The learn a word substation cypher from monolingual texts, and extend this to learn translations for 270 time expressions and 500 subtitle phrases. Their method has not been applied to full scale MT. 
%
...
Assuming that a bilingual-corpus-derived phrase table is available, we were able to recover over 82\% of BLEU loss from stripping bilingual-corpus-derived phrase-table probabilities by utilizing our monolingually-estimated features instead. We also showed that our monolingual features add 1.5 BLEU points when combined with standard bilingually estimated features.  Thus our  techniques have both stand-alone efficacy when large bilingual corpora are not available, and make a significant contribution to combined ensemble performance when they are.
