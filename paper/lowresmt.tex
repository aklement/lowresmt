...
\paraheader{Data.} In Section \ref{sect:results}, we present results on Spanish-English MT. We use the Spanish and English sections of the Europarl v5 corpus \cite{Koehn:2005}. 
%When estimating monolingual parameters, we treat the two sides of the corpus as two {\em independent monolingual} corpora.  
Our development data was comprised of {\em news-test2008} and {\em news-syscomb2009}, and our test set was {\em news-test2009} \cite{callisonburch-EtAl:2010:WMT}.  Our seed dictionary has entries for 49,795 source tokens\footnote{Translation results are robust to substantially reducing dictionary size; we omit the experiments due to space constraints.}. 

Our first set of experiments treats the two sides of the Europarl training corpus\footnote{Europarl filenames include dates, making temporal signatures easy to compute.} as two independent, monolingual corpora. We estimate contextual, temporal, and reordering features using this idealized data setup for the given phrase table. Then, in the second set of experiments, we estimate the features using truly independent corpora. We estimate contextual and temporal monolingual features using a sample of the Spanish and English Gigaword corpora\footnote{We use the entirety of the following English and Spanish Gigaword sections: Agence France-Presse (afp), Associated Press Worldstream (apw), and Xinhua News Agency (xin).}, and we estimate monolingual topic similarity and an additional contextual feature using a sample of the set of English and Spanish Wikipedia articles that contain interlingual links to one another\footnote{We use a subset of about 60,000 paired, interlingually linked pages.}. We also estimate our monolingual reordering features using the Wikipedia data. 
...
In addition to the given phrase table, the only bilingual resources that we use are a seed dictionary and a development set to tune feature weights in minimum error rate training. However, the development bitext is very small (2,553 sentences).
