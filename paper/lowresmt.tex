...
\newcite{Haghighi:2008} learn translations for only the 2000 most frequent nouns.   \newcite{Garera2009} expand beyond nouns to any part of speech, but still limit themselves to extracting the translations of the 1000 most frequent words.
...
\paraheader{Induced lexical translations.} We use a bilingual lexical induction framework that works by comparing context, time, and edit distance source and target word vectors to build a table of candidate translations for the word types (approximately 10,000 for Spanish, x for Urdu) in our development and test sets that do not have entries in our bilingual dictionary. In Section \ref{sect:exp:pt} we use this relatively small set of unigram translation pairs to supplement the above tables.
...
For reproducibility, we plan to release our software for estimating phrasal, lexical, and reordering monolingual parameters. In addition to Urdu, we have been crawling the web for monolingual resources for a number of low density languages and will make this data available to the community. The only bilingual resources that we use are seed dictionaries and a development set to tune feature weights in minimum error rate training.  However, these bitexts are very small (2,553 and 981 sentences for es-en and ur-en, respectively).  
...
\hline
Phrase 	& Monolinual  & \multicolumn{2}{|c|} {BLEU}  \\ \cline{3-4}
Table	& Features &   \multicolumn{1}{|c}{Spanish} & \multicolumn{1}{c|}{Urdu} \\ 
\hline
Dict & None & 9.09 & x\\
