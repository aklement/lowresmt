...
We extend past efforts at inducting bilingual lexicons from monolingual corpora to so that we not only extract translations of  words bit also {\it phrasal} translations. Inducing a bilingual lexicon are computationally expensive since they require $n^2$ comparisons, since each source-target word pair must be scored.  The models must  compute $|V_{s}| * |V_{t}|$ similarity scores, where $|V_{s}|$ and $|V_{s}|$ are the number of unique items in the source and target vocabularies. As we search for longer phrase pairs, this search space increases to the number of unique {\it n}-grams. Exhaustively scoring all phrase pairs up to length three requires computation scores between the set of unique unigrams, bigrams and trigrams in the source and target languages. Because this search space is very large, we aggressively prune it.  Specifically, 
