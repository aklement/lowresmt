...
\todop{Alex}{Change \figref{fig:lexinduct} and add experimental details for reproducibility.}

\begin{figure}[t]
%\vskip 0.1in
\begin{center}
\includegraphics[width=\linewidth]{../figures/lexinductnew/lexinductnew.pdf}
\caption{\todop{Alex}{Update caption: Accuracy on English and Spanish Wikipedia articles for 1,000 most frequent and 1,000 random source words.  Accuracy is the proportion of the source words with at least one correct translation in the induced top-k list of candidates.}}
\label{fig:lexinduct}
\end{center}
\vskip -0.2in
\end{figure}
...
\todop{Alex}{Add experimental details for reproducibility.}
\todop{Alex}{Add an explicit statement that we are publishing data in X languages here or in Section 5.4.}
\todop{Alex}{Actual/used dictionary size.}
\todop{Chris}{Shall we mention that we use dev to estimate features, but dev is tiny.}
