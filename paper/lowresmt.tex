...
In this work we have made use of plentiful monolingual data to induce a machine translation system, proposing one the way to reduce the dependence on the expensive and sparse parallel texts.  Specifically, we propose monolingual equivalents of the phrase-based translation model parameters extending the existing lexicon induction work.  We also introduce a novel algorithm for estimating orientation probabilities from monolingual data.  Finally, we propose methods for generating phrase tables without any sentence-aligned data.

This work is an initial step toward reducing the requirement for expensive and sparse parallel data assumed to be available by state-of-the-art statistical translation methods.  There are many directions for possible future work and remaining questions.  Monolingual data is much more available and cheap to collect than bitexts, but it remains to be seen if the size of monolingual data can afford continuous improvements without the need for a more complex translation model.
