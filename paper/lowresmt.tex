...
We score both the induced phrase table and the dictionary-composed table with monolingual features. In the case of the induced phrase table, we use the monolingual feature scores to prune the table from over 500 million phrase pairs to 1.2 million\mtodo{get final number}. We use the dictionary-compose table's inherent alignments to compute its alignment-based monolingual features [and automatic alignments on induced table if time]. We use random reordering features for both. 

We tune our feature weights using a dictionary-composed development set phrase table.\mtodo{Hopefully we show that those weights aren't much different from any other set of weights? Justify just using those for the induced table decode}. BLEU scores are given in \ref{table:new-phrase-tables}

\begin{table}
\small
\begin{center}
\label{table:new-phrase-tables}
...
\hline
\end{tabular}
\caption{Decoding with induced and dictionary-composed phrase tables}
\end{center}
\end{table}
