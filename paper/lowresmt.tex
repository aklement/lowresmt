...
\maketitle
\begin{abstract}
The parameters of statistical translation models are typically estimated from bilingual parallel corpora.  However, these resources are expensive to produce and are unavailable for most language pairs.  In this work, we estimate the parameters of a phrase-based statistical machine translation system from cheap {\emph monolingual} corpora.  In particular, we (1) extend existing research on bilingual lexicon induction to estimate lexical and phrasal translation probabilities for MT scale phrase-tables\mtodo{Anything about the novel wiki feature?}, and (2) propose a novel algorithm to estimate re-ordering probabilities from monolingual data.  We perform a feature lesion study that shows how much translation performance decreases when parameters estimated from bitext are removed and how much of that loss can be restored when monolingually-estimated equivalents are added.  We show that more than \red{XX}\% of the loss can be recovered with true monolingual data alone.
...
We then examine a realistic scenario, estimating values for an Spanish-English phrase table using truly independent monolingual corpora.  The contributions of this work as follows:
...
\begin{itemize}
  ...
  \item In \secref{sect:score} we use multiple cues present in monolingual data to estimate lexical and phrasal translation probabilities.
...
\paraheader{Topic similarity.}\mtodo{Spin this as a new feature!}  Phrases and their translations are likely to appear in articles written about the same topic in two languages.  Thus, topic or category information associated with monolingual data can also be used to indicate similarity between a phrase and its translation.  In order to score a pair of phrases, we can collect their topic signatures by counting their occurrences in each topic and then comparing the resulting vectors.  We again use the cosine similarity measure on the normalized topic signatures.  In our experiments, we use interlingual links between wikipedia articles to estimate topic similarity.  That is, we treat each linked article pair as a topic and collect counts for each phrase across all articles in its corresponding language.
...
In our experimental setup, we use a fixed set of phrase translation pairs that is extracted from a bitext, but we reestimate the translation and reordering parameters using our monolingual features. In the first set of experiments using monolingual features, we treat the two sides of our bitext as independent, monolingual corpora. That is, we throw away sentence alignments and then estimate the features based on corpora which we know have equal phrasal and temporal monolingual distributions. In the second set of experiments, we throw away this idealization and estimate the features from truly monolingual corpora. Both sets of experiments assume that a high quality phrase table is given. This assumption enables us to test the quality of our monolingual features directly. In future work, we will explore inducing the phrase table itself without using a large bitext.
