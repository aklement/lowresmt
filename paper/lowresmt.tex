...
The context vectors for words and phrases collect co-occurrence counts using a two-word window on either side.
...
\caption{Performance of monolingual features derived from truly monolingual corpora. Over 80\% of the BLEU score loss can be recovered.}
...
% ccb - squeezing for space
\vspace{-.2cm}
\subsection{Adding equivalent monolingual features estimated using {\em truly monolingual} corpora}\label{sec:results:mono}
...
Combining the monolingually estimated reordering features with the monolingually estimated similarity features recovers ({\bf M/M}) yields a total gain of {\bf 13.3 BLEU} points, or {\bf over 81\%} of the BLEU point loss that occurred when we dropped all features from the phrase table.  This is equivalent to training the standard system on a bitext with roughly 60,000 lines or nearly 2 million words  (learning curve omitted for space).
...
\newcite{Carbonell2006} described a data-driven machine translation system that used no parallel text. It produced translation lattices using a bilingual dictionary and scored them using an n-gram language model.  Their method has no notion of translation similarity aside from a  bilingual dictionary.

\newcite{ravi-knight:2011:ACL-HLT} treat MT without parallel training data as a decipherment task.  The learn a word substation cypher from monolingual texts, and extend this to learn translations for 270 time expressions and 500 subtitle phrases. Although promising, their method has not been applied to full scale MT. 

Most work on learning translations from monolingual texts only examine small numbers of frequent words. \newcite{Huang2005} and \newcite{daumeiii-jagarlamudi:2011:ACL-HLT2011} are exceptions that improve MT by mining translations for OOV words and phrases. 
