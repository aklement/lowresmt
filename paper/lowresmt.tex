...
We begin with a brief analysis of the challenges of exploiting monolingual scores in statistical machine translation.   Next, we run a series of lesion experiments in which we drop feature functions estimated from parallel data.  We then add equivalent but monolingually estimated features in reverse order to see how much of the lost performance can be regained.  Finally, we move on to phrase table induction experiments.

\subsection{Bilingual lexicon induction for SMT} \label{sect:exp:induction}

There are inherent challenges in extending lexicon induction methods to statistical machine translation.  Results reported in most of the previous literature tend to be misleading when these methods are scaled to large translation lexicons.  While the reported accuracy can be high on a small set of source language words (e.g. 100 nouns in \cite{Rapp:1995}, or 1,000 most frequent words in \cite{Koehn:2002}), it drops significantly when large translation dictionaries are needed.  In an experiment on figure \figref{fig:lexinduct}, we used the contextual cue to collect ranked lists of 500 most similar single token candidates for 1,000 most frequent and 1,000 random source language words in English and Spanish Wikipedia articles\footnote{These accuracy estimates are conservative, since a only correct translations present in our seed dictionary were counted.}.  Unsurprisingly, frequent terms have a substantially better chance of being paired up with the correct translation.  These concerns are exacerbated when we move to multi-token phrases.  As with phrase translation features estimated from parallel data, phrasal counts are sparse and similarity scores are less reliable.  Monolingually estimated lexical similarity features we introduced provide additional similarity estimates less prone to these sparseness issues.  Nevertheless, in \secref{sect:exp:replacement} we show that monolingual features are very informative in our setting; even when added alongside the feature functions derived from parallel data, the account for over a BLEU point improvement in performance (see \figref{fig:remplacement}).

Second, inducing translation lexicons over entire language vocabularies involves scoring each source with all target lexemes.  The quadratic number of computations becomes prohibitive when we move from tokens to multi word phrases.  \todo{Same argument as in \secref{sect:extract}: Maybe move relevant part of \secref{sect:extract} and \secref{sect:exp:induction} to \secref{sect:bckg:lexind}?}
...
We use the monolingual feature scores to prune the induced phrase pairs from over 500 million phrase pairs to 1.1 million and to prune the dictionary-based table from 74 million to about 500 thousand entries. We include the top three lexically induced translations for each OOV unigram.
