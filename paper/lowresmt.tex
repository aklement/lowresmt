...
\vskip 0.05in
The code for the experiments in this paper, and the monolingual data we collected and used in \secref{sect:exp:indep} along with similar corpora for 54 other low density languages is available at \url{http://anonymized}.
...
\subsection{Replacing bilingual features with features estimated from monolingual data}\label{sect:exp:lesions}
...
In this work, we make use of plentiful monolingual data to induce a machine translation system, proposing one way to reduce dependence on the expensive and sparse parallel texts assumed by state-of-the-art SMT.  Specifically, we propose monolingual equivalents of the phrase-based translation model parameters, extending existing bilingual lexicon induction research.  We also introduce an algorithm for estimating orientation probabilities from monolingual data.  Finally, we suggest methods to generate phrase tables without sentence-aligned data.
